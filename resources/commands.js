...
function validateSend(params, context) {
  ...
  try {
    var val = web3.toWei(params.args.amount.replace(",", "."), "ether");
    if (val <= 0) {
      throw new Error();
    }
  } catch (err) {
  ...
  preview: function (params, context) {
    ...
    var amount = status.components.view(
      ...
      [status.components.text(
        ...
        status.localizeNumber(params.args.amount, context.delimiter, context.separator)
      )]);
